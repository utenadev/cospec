...
### FR-002: 要件ヒアリング (`hear`)
- **機能**: 対話形式で `SPEC.md` の曖昧さを解消する。
- **対話スタイル**:
    - 原則として **「選択肢」** を提示する。
    - 各選択肢には必ず **Pros（メリット）/ Cons（デメリット）** を併記する。
    - *例: 「設定管理はどうしますか？ A: 環境変数のみ (Pros: シンプル / Cons: 型安全性低), B: Pydantic (Pros: 堅牢 / Cons: 依存増)」*
...
## 3. 非機能要件
- **LLM連携**: OpenAI / Gemini / Anthropic 等のAPIキーを環境変数で管理。
