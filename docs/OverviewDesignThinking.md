...
## 3. 開発プロセスと自己記録 (Development Process & Self-Logging)

AIエージェントが自律的に開発を進める際、その過程を人間に透明化し、後続のタスクへのコンテキストを維持することが不可欠です。

### 3.1 作業ログの自動更新 (Automatic Working Log Updates)
- **原則**: まとまった作業（機能実装、リファクタリング、設定変更など）をコミットするタイミングで、必ず `docs/WorkingLog.md` を更新する。
- **記述ルール**: 
    - 日本語で記述する。
    - 最新の情報を一番上に追記する（逆時系列）。
    - 実施した内容を箇条書きで簡潔にまとめる。
- **報告の簡素化**: ログの更新自体はエージェントが自律的に行い、ユーザーへの報告は「ログを更新した」旨を伝えるのみで良い（重複した説明を避けるため）。

### 3.2 意思決定の記録
- 設計上の重要なトレードオフや判断（SPECの解釈変更など）が発生した場合は、それも作業ログに明記し、なぜその選択をしたのかを将来のエージェントや人間が追えるようにする。

### 3.3 実装計画とタスク管理 (Implementation Plan & Checklist)
- **原則**: 実装やテストの追加・修正を行う際は、事前に `docs/PLAN.md` にチェックリストを作成し、タスクを可視化する。
- **フロー**:
    1. ユーザーとの相談・要件定義。
    2. `docs/PLAN.md` に新しいセクション（日付または機能名）を作成し、ToDoリストを列挙。
    3. 実装を進めながら、完了した項目をチェック (`[x]`) する。
    4. 全て完了したらコミットする。
- **目的**: "後出し"での手戻りを防ぎ、進捗を明確にするため。
