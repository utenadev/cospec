...
## 2026-01-07 コード品質向上リファクタリング

### 目的
コードの保守性・可読性・テスト性を向上させ、長期的な開発効率を改善するためのリファクタリングを実施。

### 計画フェーズ

#### STEP1: 型ヒントの修正とCI設定 (高優先度)
- [ ] **型安全性の向上**
...
#### STEP2: 長大な関数の分割 (高優先度)
- [ ] **`main.py`の`review()`関数分割**
  - [ ] ツール選択ロジックを`CospecConfig`に移譲
  - [ ] 49行の関数を20行以下の関数に分割
- [ ] **`test_generator.py`の`extract_test_scenarios_from_spec()`分割**
  - [ ] シナリオ抽出ロジックを別クラスに分割
  - [ ] 86行の関数をモジュール化
- [ ] **`base.py`の`run_tool()`分割**
  - [ ] プロンプト処理ロジックを小さなメソッドに分割
  - [ ] 条件分岐を可読性向上

#### STEP3: エラーハンドリングの統一 (中優先度)
- [ ] **例外型の具体化**
  - [ ] `except Exception` → 具体的な例外型に変更
  - [ ] `ToolExecutionError`等のカスタム例外作成
- [ ] **エラーメッセージの統一**
  - [ ] 日本語/英語の統一
  - [ ] エラーコード体系の導入
- [ ] **ログ出力の標準化**
...
#### STEP4: 依存性の抽象化 (中優先度)
- [x] **subprocess呼び出しの抽象化**
  - [x] `CommandExecutor`インターフェースの作成
  - [x] モック化しやすい構造へ変更
- [x] **設定管理の分離**
  - [x] CLIロジックとビジネスロジックの分離
  - [x] `main.py`の役割を明確化

#### STEP5: テストとドキュメントの充実 (低優先度)
- [ ] **テストカバレッジ向上**
  - [ ] エラーケースのテスト追加
  - [ ] 統合テストの拡充
- [ ] **ドキュメントの充実**
  - [ ] docstringの追加（特に`base.py`）
  - [ ] アーキテクチャドキュメントの作成

### 期待効果
- **型安全性**: バグ減少、IDE支援向上
- **可読性**: 関数長を20行以下に、保守性向上
- **テスト性**: カバレッジ80%+、リグレッション防止
- **一貫性**: エラーハンドリング、ログ出力の統一
